...
def _build_spatial_groups(
    ...
    Uses a regular grid of patch_size × patch_size patches, then
    ranks patches by midpoint gradient-weighted importance and assigns
    group IDs (0 = least important, G-1 = most important).

    Returns
    -------
    group_map : (1, 1, H, W) long tensor of group IDs ∈ [0, G).
    """
    device = x.device
    _, C, H, W = x.shape
    delta_x = x - baseline
    ...
    # Per-pixel importance: |grad * delta_x| summed over channels
    importance = (grad_mid * delta_x).abs().sum(dim=1, keepdim=True)  # (1,1,H,W)

    # Average importance per patch
    n_rows = (H + patch_size - 1) // patch_size
    n_cols = (W + patch_size - 1) // patch_size
    ...
    # Rank patches into G groups by importance
    sorted_patches = torch.argsort(patch_importance)
    patches_per_group = n_patches // G
    patch_to_group = torch.zeros(n_patches, dtype=torch.long, device=device)
    for g in range(G):
        lo = g * patches_per_group
        hi = (g + 1) * patches_per_group if g < G - 1 else n_patches
...
def _build_path_from_velocity_2d(
    ...
    delta_x: torch.Tensor,      # (1, C, H, W)
    V: torch.Tensor,            # (G, N) velocity schedule
    group_map: torch.Tensor,    # (1, 1, H, W) group IDs
    N: int,
) -> list[torch.Tensor]:
    """
    Construct path from grouped velocity schedule for image tensors.

    V[g, k] controls how much of group g's displacement is delivered at
    step k. Total displacement per group is preserved by normalisation.

    Returns list of N+1 image tensors (the path points).
    """
    device = baseline.device
    G = V.shape[0]
    gamma = [baseline.clone()]

    # Pre-compute V normalisation per group
    v_sums = V.sum(dim=1, keepdim=True).clamp(min=1e-12)  # (G, 1)

    for k in range(N):
        step = torch.zeros_like(baseline)
        for g in range(G):
            mask = (group_map == g).expand_as(baseline)  # (1, C, H, W)
...
def optimize_path_2d(
    ...
    Pixels are grouped into G spatial groups by importance. The velocity
    schedule V ∈ ℝ^{G×N} determines when each group's displacement is
    delivered. Optimisation uses finite-difference gradients of CV²(φ).

    Starts from the straight-line path (uniform V) to preserve the good
    spatial localization of IG, while allowing the path to deviate in
    ways that improve step fidelity.

    Cost: ~(G × N × n_iter) forward+backward passes per optimisation round.
    With G=16, N=50, n_iter=15: ~12,000 evaluations (~2 min on CPU).

    Returns list of N+1 path points.
    """
    device = x.device
    delta_x = x - baseline
    ...
    # Initialise velocity: uniform (= straight line)
    ...
    # Finite-difference gradient descent on V
    eps = 0.05
    for it in range(n_iter):
        ...
        # Stochastic FD: perturb a random subset of (g, k) per iteration
        # to reduce cost from G*N to ~G evaluations per iteration
        grad_V = torch.zeros_like(V)
        for g in range(G):
            # Sample one random step per group
    ...
    return _build_path_from_velocity_2d(baseline, delta_x, best_V,
                                         group_map, N)
...
def joint_ig(
    ...
    Unlike the previous version (which used Guided IG's path), this starts
    from the **straight-line path** and optimises small deviations via
    grouped spatial velocity scheduling. This preserves the good spatial
    localization of IG while improving step fidelity.

    Algorithm
    ---------
    1. Initialise γ = straight line, μ = uniform.
    2. For s = 1 … n_alternating:
       a. Phase 1 — optimise μ given γ  (Adam on softmax logits)
       b. Phase 2 — optimise γ given μ  (stochastic FD on velocity V)
    3. Compute attributions with final (γ, μ).

    Parameters
    ----------
    G          : number of spatial groups for path optimisation.
    patch_size : spatial patch size for grouping pixels.
    path_iter  : FD iterations per path optimisation round.
    """
    t0 = time.time()
    device = x.device
    delta_x = x - baseline
    ...
    # Initialise: straight-line path
    gamma_pts = []
    for k in range(N + 1):
        gamma_pts.append(baseline + (k / N) * delta_x)
    ...
    for s in range(n_alternating):
        ...
        if s < n_alternating - 1:
            ...
            f_vals = f_new
            d_list, df_list = d_new, df_new
            gnorms = [float(g.norm()) for g in grads]
        else:
            Q_path = Q_mu
